...
class RegisterTab(QWidget):
    ...
    def _delete_selected(self):
        ...
        resp = QMessageBox.question(
            ...
            (
                ...
                "Their revision history will be removed, but transmittal history "
                "will remain visible because snapshots are preserved.\n\n"
    ...
    def _set_check_for_filtered(self, checked: bool):
        ...
        # Collect doc_ids for filtered subset
        if checked:
            ids = {
                getattr(rows[proxy.mapToSource(proxy.index(r, COL_SELECT)).row()], "doc_id", "")
                for r in range(proxy.rowCount())
            }
        else:
            ids = set()

        # Bulk apply selection in a single call (new model API)
        if hasattr(model, "set_selected_bulk"):
            model.set_selected_bulk(ids)
        else:
            # fallback just in case, slow but shouldn’t happen
            for r in range(proxy.rowCount()):
                srow = proxy.mapToSource(proxy.index(r, COL_SELECT)).row()
                if 0 <= srow < total:
                    model._selected[srow] = checked

            if total:
                tl = model.index(0, COL_SELECT)
                br = model.index(total - 1, COL_SELECT)
                model.dataChanged.emit(tl, br, [Qt.CheckStateRole])

        # Recount once
        self._recount()

    def set_selected_bulk(self, ids_set):
        """
        Fast bulk-update of the selection column.
        ids_set: set of doc_ids to mark as selected.
        Emits exactly ONE dataChanged for the entire column.
        """
        rows = getattr(self, "_rows", [])
        sel = getattr(self, "_selected", None)

        if sel is None or len(sel) != len(rows):
            return

        # Update all flags at Python speed.
        for i, row in enumerate(rows):
            did = getattr(row, "doc_id", "")
            sel[i] = did in ids_set
    ...
    def _show_batch_import_preamble(self):
        """Modal explainer + options (non-destructive; existing text preserved)."""
        from PyQt5.QtWidgets import (
            QDialog, QVBoxLayout, QLabel, QCheckBox,
            QComboBox, QDialogButtonBox, QTextBrowser
        )
        from PyQt5.QtCore import Qt

        dlg = QDialog(self)
        dlg.setWindowTitle("Batch Import – Excel/CSV Format")

        lay = QVBoxLayout(dlg)

        # --- Existing text preserved EXACTLY ---
        text = (
            ...
            "• Empty cells are ignored for that field; rows without DOC_ID are skipped.<br><br>"
        )

        # Render existing text in a rich-text browser (non-editable)
        text_widget = QTextBrowser()
        text_widget.setHtml(text)
        text_widget.setOpenExternalLinks(False)
        text_widget.setFixedHeight(260)
        lay.addWidget(text_widget)

        # --- APPENDED NEW OPTIONS BELOW ---

        chk_add = QCheckBox("Add NEW documents if DOC_ID does not exist")
        lay.addWidget(chk_add)

        lay.addWidget(QLabel("File Type for new documents:"))
        cb_filetype = QComboBox()
        cb_filetype.addItems(["DWG", "PDF"])
        lay.addWidget(cb_filetype)

        lay.addWidget(QLabel("Status for new documents:"))
        cb_status = QComboBox()
        cb_status.addItems([
            "Not Started", "In Progress", "On Hold", "Incomplete",
            "For Review", "Complete", "Submitted"
        ])
        lay.addWidget(cb_status)
        ...
        return {
            "add_new": chk_add.isChecked(),
            "file_type": cb_filetype.currentText(),
            "status": cb_status.currentText(),
        }

    def _import_batch_updates(self):
        """
        Import batch revision/description updates from CSV/XLSX.
        Now supports optional creation of NEW documents if DOC_ID not found.
        """
        from PyQt5.QtWidgets import QFileDialog, QMessageBox
        import time

        # -------- Step 0: Show preamble (existing text preserved + appended options) -------
        opts = self._show_batch_import_preamble()
        if not opts:
            return  # cancelled

        allow_new = opts.get("add_new", False)
        new_filetype = opts.get("file_type", "DWG")
        new_status = opts.get("status", "Not Started")

        # -------- Step 1: Let user choose file --------
        path, _ = QFileDialog.getOpenFileName(
            self, "Select CSV or Excel file",
            str(self.db_path.parent if self.db_path else ""),
            "Data Files (*.csv *.xlsx)"
        )
        if not path:
            return

        # -------- Step 2: Parse updates file --------
        ...
        try:
            ...
            return
        print(f"[PROFILE] parse: {time.perf_counter() - t_parse:.4f}s")

        if not updates:
            QMessageBox.information(self, "No Data", "No valid rows found in the selected file.")
            return

        # -------- Step 3: Determine existing DOC_IDs from the model --------
        t_exist = time.perf_counter()
        rows = getattr(self.model, "_rows", [])
        existing_ids = {r.doc_id.strip().upper() for r in rows}
        print(f"[PROFILE] existing-id scan: {time.perf_counter() - t_exist:.4f}s")

        to_update = {}  # {doc_id: {revision, description}}
        to_insert = []  # list of payload dicts

        # -------- Step 4: Split into update-set and insert-set --------
        t_split = time.perf_counter()

        for doc_id, data in updates.items():
            if doc_id in existing_ids:
                # Existing document → update revision / description
                to_update[doc_id] = data
            else:
                if allow_new:
                    # New DOC_ID → create new record with default types
                    payload = {
                        "doc_id": doc_id,
                        "doc_type": "DRAWING",
                        ...
                        "latest_rev": data.get("revision", ""),
                        "is_active": 1,
                    }
                    to_insert.append(payload)

        print(f"[PROFILE] split existing/new: {time.perf_counter() - t_split:.4f}s")
        print(f"[IMPORT] will update {len(to_update)} and insert {len(to_insert)} new documents")

        # -------- Step 5: Perform DB operations --------
        from doctransmittal_sub.services.db import (
            bulk_update_docs,
            upsert_document
        )

        t_db = time.perf_counter()
        summary = bulk_update_docs(self.db_path, self.project_id, to_update)
        print(f"[PROFILE] bulk_update_docs: {time.perf_counter() - t_db:.4f}s")
        ...
        for payload in to_insert:
            try:
                upsert_document(self.db_path, self.project_id, payload)
                new_count += 1
            except Exception as e:
                print(f"[IMPORT ERROR] Failed to insert {payload['doc_id']}: {e}")
        print(f"[PROFILE] inserts: {time.perf_counter() - t_ins:.4f}s")

        # -------- Step 6: Refresh register --------
        t_reload = time.perf_counter()
        self._reload_rows()
        self.proxy.invalidateFilter()
        print(f"[PROFILE] reload: {time.perf_counter() - t_reload:.4f}s")

        # -------- Step 7: Inform user --------
        matched = summary.get("matched", 0)
        updated_rev = summary.get("updated_rev", 0)
        updated_desc = summary.get("updated_desc", 0)

        QMessageBox.information(
            self, "Import Complete",
            f"Existing documents matched: {matched}\n"
            f"Revisions updated: {updated_rev}\n"
            f"Descriptions updated: {updated_desc}\n"
            f"New documents added: {new_count}"
        )
